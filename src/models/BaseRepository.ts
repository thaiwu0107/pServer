...
import MailManager from '../microServices/MailManager';
...
export default abstract class BaseRepository {
    ...
    constructor() {
        ...
        this.socketPushManager = new SocketPushManager();
    }
    /**
     * 取得系統時間(from DB)
     *
    ...
    public async deCode(password: Buffer | object, key?: string): Promise<string> {
        if (_.isBuffer(password)) {
            ...
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'DECODE_PASSWORD_UNDEFIND_TYPE');
        }
    }
