...
export default class LobbyTexasServer extends BaseService {
    ...
    public async checkDesk(data): Promise<any> {
        ...
        try {
            ...
            const getPlayerRedisSession = await this.repository.getPlayerRedisSession(data.memberId);
            // if (_.toNumber(getPlayerRedisSession) !== _.toNumber(Constant.WHAT_THE_HACK)) {
            //     throw new Exceptions(9001, 'this member already in other table');
            // }
            ...
            const [
                ...
                // 新增規則
                this.repository.insertNLHRule(data.tc_countDown, data.smallBlind, data.bigBlind,
            ...
            const recordID =
                await this.repository.insertSessionRecord(ruleId, tableStatusId, data.type, trans);
