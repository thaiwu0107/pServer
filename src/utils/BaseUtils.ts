...
export default class BaseUtils {
    ...
    public static MemberIdToMemberSerialAndSectionSerial(memberId: IString | string) {
        if (_.size(memberId) !== 8) {
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'MemberId Size Must to be 8');
        }
    ...
    public static validateString(value: any, paramName: string) {
        if (_.isUndefined(value) || _.isString(value)) {
            ...
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL,
    ...
    public static GetCheckSum(sASCIICardSerial: string) {
        const stringArray = _.map(sASCIICardSerial, (data) => {
            ...
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'Have To All Number In Strings');
        });
        if (_.size(stringArray) !== 12) {
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'Have To Equal 12 size');
        }
    ...
    public static EncryptCard(card: string) {
        const stringArray = _.map(card, (data) => {
            ...
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, `Have To All Number In Strings(${card})`);
        });
        if (_.size(stringArray) !== 8) {
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'Have To Equal 8 size');
        }
    ...
    public static getLongCardSerialPrintFormat(card: string, algorit: boolean = false) {
        ...
        if (iChecksum <= 9) {
            ...
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'Long CardSerial Print Format Fail');
        }
    }
    ...
    public static checkNumber(value: number) {
        if (!Number.isInteger(value)) {
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, 'checkNumber() must have to integer');
        }
        // tslint:disable-next-line:no-bitwise
        return (value & 1) === 0;
    }
